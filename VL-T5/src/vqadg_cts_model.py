...
from modeling_t5_sequential_cts import VLT5
class VLT5VQADG(VLT5):
    def __init__(self, config, num_answers=None, label2ans=None):
        ...
        if config.classifier:
            print('vqadg_model_vqav2.py classifier')
            ...
            print('vqadg_model_vqav2.py not classifier')
    ...
    def train_step(self, batch):
        ...
        #如果使用语言模型
            ...
            output = self(
                input_ids=input_ids,
                vis_inputs=(vis_feats, vis_pos),
                labels=(lm_labels, q_ids, a_ids, d_ids, q_lens, a_lens, d_lens),
                return_dict=True
            )
            ...
            #print('L', L)
            loss = output['loss']
            cts_loss = output['cts_loss']
            cts_loss = cts_loss.to(device=device)
            ...
            loss = loss.mean()
            #cts
            total_loss = 0.7 * loss + 0.3 * cts_loss
            #raw
            #total_loss = loss
        result = {
            'loss': total_loss,
