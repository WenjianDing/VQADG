    ...
    def forward(
        ...
        **kwargs,
        ...
        if encoder_outputs is None:
            ...
            encoder_outputs, vis_embeds = self.encoder(
                input_ids=input_ids,
        ...
        # b = labels[:, 4:7]
        #
        # labels1_embeds = self.decoder.embed_tokens(a)
        # labels2_embeds = self.decoder.embed_tokens(b)
        # print('labels1 embed shape', labels1_embeds.size())
        # print('labels2 embed shape', labels2_embeds.size())
        #calculate the average of the label embedding
        # a, b = labels.size()
        # labels_tensor = torch.ones(a, b, 768)
        # for i in range((b//8)+1):
        #     try:
        #         labels_piece = labels.split(8, dim=1)[i]
        #         print('label piece shape', labels_piece.size())
        #         labels_embeds = self.decoder.embed_tokens(labels_piece)
        #         piece_l = labels_piece.size()[1]
        #         #labels_tensor[:, i*8:i*8+piece_l, :] = labels_embeds
        #         labels_tensor[:, :piece_l, :] = labels_embeds
        #     except:
        #         continue
        # labels_embeds_mean = torch.mean(labels_tensor, dim=1)
        # print('labels tensor', labels_tensor.size())
        # print('labels_embeds_mean', labels_embeds_mean.size())
        ...
        image_mean = torch.mean(vis_embeds, dim=1)
        sequence_norm = sequence_mean/sequence_mean.norm(dim=-1, keepdim=True)
        image_norm = image_mean/image_mean.norm(dim=-1, keepdim=True)
        scale = np.exp(3)
        logits_per_image = scale * image_norm @ sequence_norm.t()
        ...
        # print('image size', logits_per_image.size())
        # print('labels', labels.size())
        cts_loss = (F.cross_entropy(logits_per_image, cts_labels)+F.cross_entropy(logits_per_text, cts_labels))/2

        #similarity = (100.0 * image_norm @ sequence_norm.T).softmax(dim=-1)
