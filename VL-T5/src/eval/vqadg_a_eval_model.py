...
class VLT5VQADG(VLT5):
    ...
    def train_step(self, batch):
        ...
        random_indices = np.random.permutation(B)
        #shuffle text
        if mode == 0:
            negative_input_ids2 = input_ids2[random_indices, :].to(device)
            total_input_ids2 = torch.cat((input_ids, negative_input_ids2), 1)
            negative_output = self(
                input_ids=total_input_ids2,
                vis_inputs=(vis_feats, vis_pos),
                decoder_input_ids=decoder_input_ids,
                output_hidden_states=True,
                return_dict=True
        ...
        if mode == 1:
            negative_vis_feats = vis_feats[random_indices, :, :].to(device)
            negative_vis_pos = vis_pos[random_indices, :, :].to(device)
            negative_output = self(
                input_ids=total_input_ids,
                vis_inputs=(negative_vis_feats, negative_vis_pos),
                decoder_input_ids=decoder_input_ids,
                output_hidden_states=True,
                return_dict=True
            )
        ...
        if mode == 2:
            negative_input_ids2 = input_ids2[random_indices, :].to(device)
            total_input_ids2 = torch.cat((input_ids, negative_input_ids2), 1)
            random_indices2 = np.random.permutation(B)
            negative_vis_feats = vis_feats[random_indices2, :, :].to(device)
            negative_vis_pos = vis_pos[random_indices2, :, :].to(device)
            negative_output = self(
                input_ids=total_input_ids2,
                vis_inputs=(negative_vis_feats, negative_vis_pos),
                decoder_input_ids=decoder_input_ids,
        ...
        #print('mode', mode)
        neg_last_layer_hidden_state = negative_output.decoder_hidden_states[-1]
        neg_last_hidden_state = neg_last_layer_hidden_state.view(B, -1, self.config.d_model)[:, -1]
        neg_logit = self.answer_head(neg_last_hidden_state)


        total_logit = torch.cat((logit, neg_logit), dim=0)
        neg_targets = target.new_zeros((B, 1)).to(device)
        total_targets = torch.cat((target, neg_targets), dim=0)
