...
class VLT5VQADG(VLT5):
    ...
    def train_step(self, batch, trainer1):
        ...
        #如果使用语言模型
            ...
            answer_token_tensor = result['token_ids']
            a, b = answer_token_tensor.size()
            if int(b) > 4:
                answer_token_tensor = answer_token_tensor[:, 4:]
            #get question token id
            question_token_tensor = input_ids[:, 4:]
            # print('question shape', question_token_tensor.size())
            # print('answer shape', answer_token_tensor.size())
            total_token_tensor = torch.cat((question_token_tensor, answer_token_tensor), 1)
            new_batch = {}
            new_batch['input_ids'] = total_token_tensor.to(device)
            new_batch['vis_feats'] = vis_feats.to(device)
            new_batch['boxes'] = vis_pos.to(device)
            score_predicted = trainer1.model.module.test_step(new_batch)['logit'].to(device=device)
            score_mean = score_predicted.mean()
            loss2 = 1 - score_mean
