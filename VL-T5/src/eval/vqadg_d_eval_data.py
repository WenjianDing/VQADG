...
class VQADGFineTuneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        answer = datum['answer']
        mc = datum['multiple_choices']
        target = 1
        input_ids = self.tokenizer.encode(question, max_length=20, truncation=True)
        input_ids2 = self.tokenizer.encode(answer, max_length=10, truncation=True)
        #distractors
        input_ids3 = self.tokenizer.encode(mc[0], max_length=10, truncation=True)
        input_ids4 = self.tokenizer.encode(mc[1], max_length=10, truncation=True)
        input_ids5 = self.tokenizer.encode(mc[2], max_length=10, truncation=True)
        question_id = datum['qa_id']
        out_dict['qa_id'] = question_id
        out_dict['input_ids'] = torch.LongTensor(input_ids)
        out_dict['input_ids2'] = torch.LongTensor(input_ids2)
        ...
        out_dict['input_length2'] = len(input_ids2)
    ...
    def collate_fn(self, batch):
        ...
        S_W_L2 = max(entry['input_length2'] for entry in batch)
        input_ids2 = torch.ones(B, S_W_L2, dtype=torch.long) * self.tokenizer.pad_token_id
        ...
        if 'target' in batch[0]:
            # targets = []
            targets = torch.zeros(B, 1, dtype=torch.float)
        ...
        for i, entry in enumerate(batch):
            ...
            input_ids2[i, :entry['input_length2']] = entry['input_ids2']
        ...
        batch_entry['input_ids2'] = input_ids2
