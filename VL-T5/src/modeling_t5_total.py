...
class VLT5(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        **kwargs,
        ...
        for i in range(bs):
            label = labels[i]
            ind = int(torch.min(label, 0)[1])
            label_piece = label[:ind]
        ...
        logits2_per_image = scale * image_norm @ sequence_norm.t()
        logits2_per_text = logits2_per_image.t()
        ...
        cts_loss2 = (F.cross_entropy(logits2_per_image, cts_labels) + F.cross_entropy(logits2_per_text, cts_labels)) / 2
...
class VLSeq2SeqLMOutput(ModelOutput):
    ...
    cts_loss1: torch.FloatTensor = None
    cts_loss2: torch.FloatTensor = None
