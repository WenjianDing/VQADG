...
class VQADGFineTuneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # target = 'q:' + question
        #'vqadg:type'生成'q:question','q:question'生成'a:answer','q:question;a:answer'生成'd:distractors'
        # 会有区别吗？
        input = 'q: ' + question + ' ;a: ' + answer
        target = 'd: ' + str(multiple_choices)
        input_ids = self.tokenizer.encode(f'{input}', max_length=60, truncation=True)
        input_ids2 = self.tokenizer.encode(question, max_length=20, truncation=True)
        ...
        target_ids = self.tokenizer.encode(target, max_length=60, truncation=True)
        out_dict['target_ids'] = torch.LongTensor(target_ids)
        out_dict['target_length'] = len(target_ids)
        # print('sent', sent)
        # print('input id', input_ids)
        # print('input id length', len(input_ids))
        # print('target', target)
        # print('target id', target_ids)
        # print('target id length', len(target_ids))
    ...
    def collate_fn(self, batch):
        ...
        S_W_L1 = max(entry['input_length'] for entry in batch)
        input_ids = torch.ones(B, S_W_L1, dtype=torch.long) * self.tokenizer.pad_token_id
