...
class VLT5(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        **kwargs,
        ...
        answers_embeds_mean = torch.zeros(bs, 768)
        for i in range(bs):
            answer_i = ans_labels[i]
            ind = int(torch.min(answer_i, 0)[1])
            answer_piece = answer_i[:ind]
            answer_piece_embed = self.decoder.embed_tokens(answer_piece)
            answers_piece_mean = torch.mean(answer_piece_embed, dim=0)
            answers_embeds_mean[i, :] = answers_piece_mean
        answers_embeds_mean = answers_embeds_mean/answers_embeds_mean.norm(dim=-1, keepdim=True)
        #similarity = (100.0 * labels_embeds_mean @ answers_embeds_mean.T).softmax(dim=-1)
        #s_max = torch.argmax(similarity, dim=1)
        #print('similarity', s_max)
        ...
        # contrastive learning with image
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        #print('sequence output shape', sequence_output.size())
        ...
        #predicted distractors and gt distractors similarity (about to be 1)
        logits_per_gt = scale * gt_mean @ sequence_norm.t()
        #logits_per_image = image_norm @ sequence_norm.t()
        logits_per_text = logits_per_gt.t()
        bs = sequence_output.size()[0]
        cts_labels = torch.arange(bs).long().to(device)
        #cts_loss1 = (F.cross_entropy(logits_per_gt, cts_labels)+F.cross_entropy(logits_per_text, cts_labels))/2
        #similarity1 = (100.0 * gt_mean @ sequence_norm.T).softmax(dim=-1)
        #similarity2 = (100.0 * sequence_norm @ gt_mean.T).softmax(dim=-1)
        #print('similarity1',similarity1.size())
        #print('similarity1', similarity1)
        #print('1/2 similarity1', 0.5 * similarity1)
        #print('similarity2', similarity2.size())
        #print('similarity2', similarity2)
        #print('1/2 similarity2', 0.5 * similarity2)
        #predicted distractors and gt answer similarity (about to be 0 to 1 (set 0.5))
        gt_mean2 = answers_embeds_mean.to(device)
        logits_per_gt2 = scale * gt_mean2 @ sequence_norm.t()
        logits_per_text2 = logits_per_gt2.t()
        #similarity1 = (100.0 * gt_mean2 @ sequence_norm.T).softmax(dim=-1)
        #similarity2 = (100.0 * sequence_norm @ gt_mean2.T).softmax(dim=-1)
        cts_loss = (F.cross_entropy(logits_per_gt, cts_labels)+F.cross_entropy(logits_per_text, cts_labels))/2 + (F.cross_entropy(2 * logits_per_gt2, cts_labels)+F.cross_entropy(2 * logits_per_text2, cts_labels))/2
        # cts_loss = (F.cross_entropy(logits_per_gt, cts_labels)+F.cross_entropy(logits_per_text, cts_labels))/2 + (F.cross_entropy(similarity1, cts_labels)+F.cross_entropy(similarity2, cts_labels))/2
