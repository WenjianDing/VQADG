...
vqadg_dir = dataset_dir.joinpath('vqadg')
...
class VQADGFineTuneDataset(Dataset):
    def __init__(self, split='train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train'):
        ...
        if self.args.classifier:
            print('vqadg_data.py classifier')
        else:
            print('vqadg_data.py not classifier')
    ...
    def __getitem__(self, idx):
        ...
        input_ids_q = self.tokenizer.encode(question, max_length=30, truncation=True)
        input_ids_a = self.tokenizer.encode(answer, max_length=20, truncation=True)
        input_ids3 = self.tokenizer.encode(multiple_choices[0], max_length=20, truncation=True)
        input_ids4 = self.tokenizer.encode(multiple_choices[1], max_length=20, truncation=True)
        input_ids5 = self.tokenizer.encode(multiple_choices[2], max_length=20, truncation=True)
        out_dict['input_ids_q'] = torch.LongTensor(input_ids_q)
        out_dict['input_q_length'] = len(input_ids_q)
        out_dict['input_ids_a'] = torch.LongTensor(input_ids_a)
        out_dict['input_a_length'] = len(input_ids_a)
        out_dict['input_ids3'] = torch.LongTensor(input_ids3)
        out_dict['input_ids4'] = torch.LongTensor(input_ids4)
        out_dict['input_ids5'] = torch.LongTensor(input_ids5)
        out_dict['input_length3'] = len(input_ids3)
        out_dict['input_length4'] = len(input_ids4)
        out_dict['input_length5'] = len(input_ids5)
    ...
    def collate_fn(self, batch):
        ...
        S_W_L_q = max(entry['input_q_length'] for entry in batch)
        input_ids_q = torch.ones(B, S_W_L_q, dtype=torch.long) * self.tokenizer.pad_token_id
        S_W_L_a = max(entry['input_a_length'] for entry in batch)
        input_ids_a = torch.ones(B, S_W_L_a, dtype=torch.long) * self.tokenizer.pad_token_id
        S_W_L3 = max(entry['input_length3'] for entry in batch)
        input_ids3 = torch.ones(B, S_W_L3, dtype=torch.long) * self.tokenizer.pad_token_id
        S_W_L4 = max(entry['input_length4'] for entry in batch)
        input_ids4 = torch.ones(B, S_W_L4, dtype=torch.long) * self.tokenizer.pad_token_id
        S_W_L5 = max(entry['input_length5'] for entry in batch)
        input_ids5 = torch.ones(B, S_W_L5, dtype=torch.long) * self.tokenizer.pad_token_id
        ...
        for i, entry in enumerate(batch):
            ...
            input_ids_q[i, :entry['input_q_length']] = entry['input_ids_q']
            input_ids_a[i, :entry['input_a_length']] = entry['input_ids_a']
            input_ids3[i, :entry['input_length3']] = entry['input_ids3']
            input_ids4[i, :entry['input_length4']] = entry['input_ids4']
            input_ids5[i, :entry['input_length5']] = entry['input_ids5']
        ...
        batch_entry['input_ids_q'] = input_ids_q
        batch_entry['input_ids_a'] = input_ids_a
        batch_entry['input_ids3'] = input_ids3
        batch_entry['input_ids4'] = input_ids4
        batch_entry['input_ids5'] = input_ids5
        if 'target_ids' in batch[0]:
...
def get_loader(args, split='train', mode='train',
    ...
    #_dset = VQADGDataset(split, verbose)
