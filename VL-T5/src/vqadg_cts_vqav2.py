...
from vqadg_cts_data_vqav2 import get_loader
...
class Trainer(TrainerBase):
    ...
    def __init__(self, args, train_loader=None, train=True):
        super().__init__(
            args,
        ...
        from vqadg_cts_model import VLT5VQADG, VLBartVQADG

        model_kwargs = {}
        if 't5' in args.backbone:
            model_class = VLT5VQADG
        elif 'bart' in args.backbone:
            model_class = VLBartVQADG
    ...
    def train(self):
        if self.verbose:
            ...
            if 't5' in self.args.backbone:
                if self.args.use_vision:
                    project_name = "VLT5_VQADG"
                else:
                    project_name = "T5_VQADG"
            elif 'bart' in self.args.backbone:
                if self.args.use_vision:
                    project_name = "VLBart_VQADG"
                else:
                    project_name = "Bart_VQADG"
        ...
        for epoch in range(self.args.epochs):
            ...
            for step_i, batch in enumerate(self.train_loader):
                ...
                loss = results['loss']
                loss1 = results['lm loss']
                loss2 = results['cts loss']
                ...
                if self.verbose:
                    ...
                    desc_str += f' | Lossd {loss_meter.val:4f}'
                    desc_str += f' | LM Loss {l1:4f}'
                    desc_str += f' | CTS Text Loss {l2:4f}'
...
def main_worker(gpu, args):
    ...
    print(f'Building train loader for vqav2 at GPU {gpu}')
