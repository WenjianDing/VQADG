...
class JointEncoder(T5Stack):
    ...
    def forward(
        ...
        #将image embed和sentence embed concat在一起
...
class VLT5(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        **kwargs,
        ...
        a_lens = labels[5]
        d_lens = labels[6]
        qad_lens = q_lens + a_lens + d_lens
        q_ratio = q_lens / qad_lens
        a_ratio = a_lens / qad_lens
        d_ratio = d_lens / qad_lens
        #print(q_ratio, a_ratio, d_ratio)
        ...
        # calculate the average of the label embedding
        bs = lm_labels.size()[0]
        labels_embeds_mean = torch.zeros(bs, 768)
        for i in range(bs):
            label_i = lm_labels[i]
            ind = int(torch.min(label_i, 0)[1])
            label_piece = label_i[:ind]
            label_piece_embed = self.decoder.embed_tokens(label_piece)
            labels_piece_mean = torch.mean(label_piece_embed, dim=0)
            labels_embeds_mean[i, :] = labels_piece_mean
        labels_embeds_mean = labels_embeds_mean/labels_embeds_mean.norm(dim=-1, keepdim=True)

        q_embeds_mean = torch.zeros(bs, 768)
        for i in range(bs):
            q_i = q_labels[i]
            ind = int(torch.min(q_i, 0)[1])
            q_piece = q_i[:ind]
            q_piece_embed = self.decoder.embed_tokens(q_piece)
            q_piece_mean = torch.mean(q_piece_embed, dim=0)
            q_embeds_mean[i, :] = q_piece_mean
        q_embeds_mean = q_embeds_mean/q_embeds_mean.norm(dim=-1, keepdim=True)

        a_embeds_mean = torch.zeros(bs, 768)
        for i in range(bs):
            a_i = a_labels[i]
            ind = int(torch.min(a_i, 0)[1])
            a_piece = a_i[:ind]
            a_piece_embed = self.decoder.embed_tokens(a_piece)
            a_piece_mean = torch.mean(a_piece_embed, dim=0)
            a_embeds_mean[i, :] = a_piece_mean
        a_embeds_mean = a_embeds_mean/a_embeds_mean.norm(dim=-1, keepdim=True)

        d_embeds_mean = torch.zeros(bs, 768)
        for i in range(bs):
            d_i = d_labels[i]
            ind = int(torch.min(d_i, 0)[1])
            d_piece = d_i[:ind]
            d_piece_embed = self.decoder.embed_tokens(d_piece)
            d_piece_mean = torch.mean(d_piece_embed, dim=0)
            d_embeds_mean[i, :] = d_piece_mean
        d_embeds_mean = d_embeds_mean/d_embeds_mean.norm(dim=-1, keepdim=True)
        ...
        sequence_norm = sequence_mean/sequence_mean.norm(dim=-1, keepdim=True)
        scale = np.exp(3)
        gt_mean = labels_embeds_mean.to(device)
        logits_per_gt = scale * gt_mean @ sequence_norm.t()
        logits_per_text = logits_per_gt.t()
        bs = sequence_output.size()[0]
        cts_labels = torch.arange(bs).long().to(device)
        l = sequence_output.size()[1]
        q_len = int(l * q_ratio)
        a_len = int(l * a_ratio)
        d_len = l - q_len - a_len
        q_output = sequence_output[:, :q_len, :]
        q_mean = torch.mean(q_output, dim=1)
        q_norm = q_mean / q_mean.norm(dim=-1, keepdim=True)
        gt_q_mean = q_embeds_mean.to(device)
        logits_per_gt_q = scale * gt_q_mean @ q_norm.t()
        logits_per_text_q = logits_per_gt_q.t()

        a_output = sequence_output[:, q_len:q_len+a_len, :]
        a_mean = torch.mean(a_output, dim=1)
        a_norm = a_mean / a_mean.norm(dim=-1, keepdim=True)
        gt_a_mean = a_embeds_mean.to(device)
        logits_per_gt_a = scale * gt_a_mean @ a_norm.t()
        logits_per_text_a = logits_per_gt_a.t()

        d_output = sequence_output[:, q_len+a_len:q_len+a_len+d_len, :]
        d_mean = torch.mean(d_output, dim=1)
        d_norm = d_mean / d_mean.norm(dim=-1, keepdim=True)
        gt_d_mean = d_embeds_mean.to(device)
        logits_per_gt_d = scale * gt_d_mean @ d_norm.t()
        logits_per_text_d = logits_per_gt_d.t()


        logits_per_gt_d_a = scale * gt_a_mean @ d_norm.t()
        logits_per_text_d_a = logits_per_gt_d_a.t()

        # cts_loss = (F.cross_entropy(logits_per_gt, cts_labels) + F.cross_entropy(logits_per_text, cts_labels)) / 2 + \
        #            (F.cross_entropy(logits_per_gt_q, cts_labels) + F.cross_entropy(logits_per_text_q, cts_labels)) / 2 + \
        #            (F.cross_entropy(logits_per_gt_a, cts_labels) + F.cross_entropy(logits_per_text_a, cts_labels)) / 2 + \
        #            (F.cross_entropy(logits_per_gt_d, cts_labels) + F.cross_entropy(logits_per_text_d, cts_labels)) / 2 + \
        #            (F.cross_entropy(2 * logits_per_gt_d_a, cts_labels) + F.cross_entropy(2 * logits_per_text_d_a, cts_labels)) / 2
        cts_loss = (F.cross_entropy(logits_per_gt_q, cts_labels) + F.cross_entropy(logits_per_text_q, cts_labels)) / 2 + \
                   (F.cross_entropy(logits_per_gt_a, cts_labels) + F.cross_entropy(logits_per_text_a, cts_labels)) / 2 + \
                   (F.cross_entropy(logits_per_gt_d, cts_labels) + F.cross_entropy(logits_per_text_d, cts_labels)) / 2 + \
                   (F.cross_entropy(2 * logits_per_gt_d_a, cts_labels) + F.cross_entropy(2 * logits_per_text_d_a, cts_labels)) / 2
        ...
        #在推理的时候，一次只生成[1,1,768]，也就是一个单词，然后不断循环生成一句话。
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs):
        ...
        #print('output decoder input ids', output['decoder_input_ids'].size())
        # [1,1]
        #print('output past_key_values', output['past_key_values'].size())
        #print('output encoder_outputs', output['encoder_outputs'].size())
...
@dataclass
#cts_loss: torch.FloatTensor = None
